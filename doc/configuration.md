...
| `exclusive_zone` | Integer | `60` | The height of the reserved area on the screen edge (macOS-like spacing). Only active if `overlay` is `false`. |
...
You can provide a custom CSS file at `~/.config/hyprdock/style.css` to override any default styles.
...
### Context Menus
- `.context-menu`: The right-click menu container.
- `.context-menu-item`: Individual items within the menu.
